...
class SettingsService {
    ...
    const STYLE_URL      = 'unsplash/style/url';
    const DEFAULT_UNSPLASH_URL = 'https://source.unsplash.com/random/featured/?nature';
    ...
    /**
     * Get the url for unsplash
     * If not set, returns
     *
     * @return String
     */
    public function getServerStyleUrl(){
        $url=$this->config->getAppValue($this->appName, self::STYLE_URL);
        if(strlen($url)<8){
            $url= self::DEFAULT_UNSPLASH_URL;
}
